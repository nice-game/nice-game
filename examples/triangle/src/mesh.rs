...
impl MeshBatch {
	pub fn new(shared: &MeshBatchShared, target: &mut RenderTarget) -> Self {
		let recreated = Arc::<AtomicBool>::default();
		target.register(recreated.clone());

		Self {
			device: shared.device.clone(),
			render_pass: shared.render_pass.clone(),
			pipeline: Arc::new(
				GraphicsPipeline::start()
					.vertex_input_single_buffer::<TriangleVertex>()
					.vertex_shader(shared.vertex_shader.main_entry_point(), ())
					.triangle_list()
					.viewports_dynamic_scissors_irrelevant(1)
					.fragment_shader(shared.fragment_shader.main_entry_point(), ())
					.render_pass(Subpass::from(shared.render_pass.clone(), 0).expect("failed to create subpass"))
					.build(shared.device.clone())
...
impl Drawable for MeshBatch {
	fn commands(
		...
		let mut command_buffer = AutoCommandBufferBuilder::primary_one_time_submit(self.device.clone(), queue_family)
			.unwrap()
...
impl MeshBatchShared {
	pub fn new(device: Arc<Device>, format: Format) -> Self {
		...
		let vertex_shader = vs::Shader::load(device.clone()).expect("failed to load shader module");
		let fragment_shader = fs::Shader::load(device.clone()).expect("failed to load shader module");
